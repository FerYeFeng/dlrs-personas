...
function parseObjGeometries(text) {
  const vertices = [];
  const normals = [];
  const groups = new Map();
  let current = null;

  const ensureGroup = (name = 'default') => {
    if (!groups.has(name)) groups.set(name, { name, positions: [], normals: [] });
    return groups.get(name);
  };

  const readIndex = (raw, length) => {
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (parts[0] === 'v') {
      ...
      current = ensureGroup(parts.slice(1).join(' ') || 'default');
    } else if (parts[0] === 'f') {
      current ||= ensureGroup();
      const face = parts.slice(1).map((token) => {
        const [v, , n] = token.split('/');
        return {
          v: readIndex(v, vertices.length),
          n: n ? readIndex(n, normals.length) : -1
        };
      });
      for (let i = 1; i < face.length - 1; i++) {
        for (const ref of [face[0], face[i], face[i + 1]]) {
          const p = vertices[ref.v];
          if (!p) continue;
          current.positions.push(p.x, p.y, p.z);
          const normal = normals[ref.n];
          if (normal) current.normals.push(normal.x, normal.y, normal.z);
        }
      }
    }
  }

  return [...groups.values()].filter((group) => group.positions.length).map((group) => {
...
async function loadObjGroup(url, materialForGroup) {
  const res = await fetch(url);
  if (!res.ok) throw new Error(`Failed to load OBJ: ${url}`);
  const group = new THREE.Group();
  group.name = 'headphones-obj';
  for (const geo of parseObjGeometries(await res.text())) {
    const mesh = new THREE.Mesh(geo, materialForGroup(geo.name));
    mesh.name = geo.name;
    group.add(mesh);
  }
  return group;
...
// Cube + Headphones 调参区：改这里的数字，刷新页面即可。
// x: 左右，y: 上下，z: 前后；单位是模型单位。
const CUBE_HEADPHONES_TUNING = {
  cube: {
    size: 0.86,
...
function createCubeHeadphonesModel(options = {}) {
  const { castShadow = true, receiveShadow = true } = options;
  const M = makeMaterials();
  const T = CUBE_HEADPHONES_TUNING;
  const cubeBaseColor = new THREE.Color(T.colors.cube);
  const cubeBaseHsl = {};
  cubeBaseColor.getHSL(cubeBaseHsl);
  const cubeTargetColor = new THREE.Color(T.colors.cube);
  const cubeTargetEmissive = new THREE.Color(0x0e4a72);
  const nodes = {}, meshes = {}, sockets = {};
  const root = new THREE.Group();
  root.name = 'cube-headphones-3d';

  const bodyPivot = new THREE.Group();
  bodyPivot.name = 'cube-headphones-assembly';
  root.add(bodyPivot);
  nodes['body-assembly'] = bodyPivot;

  const toy = new THREE.Group();
  toy.name = 'cube-headphones-frame';
  bodyPivot.add(toy);
  nodes['body-frame'] = toy;
  ...
  const cube = new THREE.Mesh(new THREE.BoxGeometry(T.cube.size, T.cube.size, T.cube.size, 4, 4, 4), cubeMaterial);
  cube.name = 'cube-body';
  cube.position.set(T.cube.position.x, T.cube.position.y, T.cube.position.z);
  cube.castShadow = castShadow;
  cube.receiveShadow = receiveShadow;
  toy.add(cube);
  meshes['cube-body'] = cube;

  const edges = new THREE.LineSegments(
    new THREE.EdgesGeometry(cube.geometry),
    new THREE.LineBasicMaterial({ color: T.colors.cubeEdges, transparent: true, opacity: 0.48 })
  );
  edges.name = 'cube-edges';
  edges.position.copy(cube.position);
  toy.add(edges);
  meshes['cube-edges'] = edges;

  const headphoneMaterial = new THREE.MeshPhysicalMaterial({
    color: T.colors.headphones,
    roughness: 0.18,
    metalness: 0.18,
    clearcoat: 0.45,
  ...
  const headphoneGrayMaterial = new THREE.MeshPhysicalMaterial({
    color: T.colors.headphones1,
    roughness: 0.28,
    metalness: 0.08,
    clearcoat: 0.28,
  ...
  headphones.position.set(T.headphones.position.x, T.headphones.position.y, T.headphones.position.z);
  headphones.rotation.set(T.headphones.rotation.x, T.headphones.rotation.y, T.headphones.rotation.z);
  toy.add(headphones);
  nodes.headphones = headphones;
  loadObjGroup('./3d/assets/Headphones.obj', (name) => (
    name === 'Headphones_1' ? headphoneGrayMaterial : headphoneMaterial
  )).then((mesh) => {
    mesh.traverse((child) => {
    ...
    mesh.position.set(-cx, T.headphones.centerToObjMiddle ? -cy : 0, -cz);
    mesh.scale.setScalar(T.headphones.scale);
    headphones.add(mesh);
    meshes['headphones-obj'] = mesh;
  }).catch(() => {
    const fallback = new THREE.Mesh(new THREE.TorusGeometry(0.46, 0.045, 18, 64, Math.PI), headphoneMaterial);
    fallback.name = 'headphones-fallback';
    fallback.rotation.z = Math.PI;
    fallback.position.y = -0.18;
    fallback.castShadow = castShadow;
    fallback.receiveShadow = receiveShadow;
    headphones.add(fallback);
    meshes['headphones-fallback'] = fallback;
  });

  const hook = new THREE.Mesh(new THREE.TorusGeometry(T.stringHook.radius, T.stringHook.tubeRadius, 10, 28), M.black);
  hook.name = 'headphones-string-hook';
  hook.rotation.x = Math.PI / 2;
  hook.position.set(T.stringHook.position.x, T.stringHook.position.y, T.stringHook.position.z);
  hook.castShadow = castShadow;
  headphones.add(hook);
  meshes['headphones-string-hook'] = hook;

  sockets['headphones-top'] = new THREE.Object3D();
  sockets['headphones-top'].position.set(T.stringHook.position.x, T.stringHook.position.y, T.stringHook.position.z);
  headphones.add(sockets['headphones-top']);

  const handle = new THREE.Group();
  handle.name = 'handle-assembly';
  handle.visible = false;
  root.add(handle);
  nodes['handle-assembly'] = handle;
  sockets['stick-waist'] = new THREE.Object3D();
  sockets['stick-waist'].position.set(0, 0, 0);
  handle.add(sockets['stick-waist']);

  let stringMesh = null;
  const rebuildString = () => {
    const a = sockets['headphones-top'].getWorldPosition(new THREE.Vector3());
    ...
    const sag = state && state.mode === 'driven' ? Math.max(0.01, state.sag || 0) : 0.025;
    const curve = new THREE.CatmullRomCurve3(catenaryPoints(a, b, sag, 28));
  ...
  const state = { mode: 'idle', t: 0, sag: 0, spinGlow: 0 };
  root.userData._state = state;
  root.updateMatrixWorld(true);
  rebuildString();

  const setCubeHue = (hue, saturation = 0.9, lightness = 0.58, emissiveLightness = 0.34, smooth = 1) => {
    cubeTargetColor.setHSL(hue, saturation, lightness);
    cubeTargetEmissive.setHSL(hue, saturation, emissiveLightness);
    cube.material.color.lerp(cubeTargetColor, smooth);
    cube.material.emissive.lerp(cubeTargetEmissive, smooth);
  };

  root.userData.setExplode = () => {};
  root.userData.setMode = (mode) => { state.mode = mode; };
  root.userData.drivePose = (pose) => {
    state.mode = 'driven';
    state.sag = pose.sag;
    handle.position.set(pose.stick.x, pose.stick.y, 0);
    bodyPivot.position.set(pose.tube.x, pose.tube.y, 0);
    bodyPivot.rotation.set(pose.tilt * 0.7, pose.spin, pose.headAngle, 'ZYX');
    toy.rotation.set(
      Math.sin(pose.spin * 0.7) * 0.12,
      pose.spin * 0.22,
      Math.cos(pose.spin * 0.45) * 0.1
    );
    state.spinGlow = Math.min(1, Math.abs(pose.omega || 0) / 10);
    const hueRate = T.colorCycle.baseSpeed + Math.min(T.colorCycle.maxSpeedBoost, Math.abs(pose.omega || 0) * T.colorCycle.speedBySpin);
    const now = pose.now || 0;
    const hue = (cubeBaseHsl.h + now * hueRate) % 1;
    setCubeHue((hue + 1) % 1, 0.88, 0.58, 0.28, T.colorCycle.smoothing);
    cube.material.emissiveIntensity = 0.18 + state.spinGlow * 0.4;
    edges.material.opacity = 0.42 + state.spinGlow * 0.42;
    root.updateMatrixWorld(true);
    rebuildString();
  };
  root.userData.setSing = (v) => {
    edges.material.opacity = Math.max(edges.material.opacity, 0.38 + v * 0.2);
  };
  root.userData.tick = (dt, t) => {
    state.t = t;
    if (state.mode === 'driven') return;
    toy.rotation.y = Math.sin(t * 0.7) * 0.12;
    setCubeHue(cubeBaseHsl.h, cubeBaseHsl.s, cubeBaseHsl.l, 0.24);
...
export function createZhuzhiliaoModel(options = {}) {
  return createCubeHeadphonesModel(options);
