...
def save_db(db, merged):
    ...
    write_json(USERS_EXPORT_PATH, db["gasUsers"], indent=2)
...
def main():
    ...
    for uid in range(start_uid, args.end + 1):
        ...
        if checked % max(1, args.status_every) == 0:
            ...
            print(
                f"Progress: checked {checked}/{total} ({percent}%), current UID {uid}, cached {len(merged)}, found this run {found}",
                flush=True,
            )
    ...
    print(f"GAS sync done: checked {checked}, cached {len(merged)}, found this run {found}", flush=True)
    FETCHER.close()
